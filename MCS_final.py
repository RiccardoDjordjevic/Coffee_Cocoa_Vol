...
def load_and_combine_data():
    ...
    try:
        ...
        elasticnet_df = pd.read_csv('elasticnet_clean_fold_losses_mcs.csv')
        lasso_df = pd.read_csv('lasso_clean_fold_losses_mcs.csv')
        ols_df = pd.read_csv('ols_fold_losses_mcs.csv')
        bagging_df = pd.read_csv('bagging_clean_fold_losses_mcs.csv')
        ...
        print(f"- Unique combinations of (volatility_type, horizon): {len(combined_df.groupby(['volatility_type', 'horizon']))}")
...
def create_mcs_results_summary(results_df):
    ...
    print("-" * 40)
    try:
        avg_mcs_size = mcs_df.groupby(['volatility_type', 'horizon', 'loss_metric'])['n_models_in_mcs'].first().groupby('loss_metric').mean()
    ...
    try:
        ...
        if not results_df.empty:
            ...
            if not mcs_only_df.empty:
                ...
                mcs_summary_list = []
                for (vol_type, horizon, metric), group in mcs_only_df.groupby(['volatility_type', 'horizon', 'loss_metric']):
                ...
                if mcs_summary_list:
                    ...
                    print("-" * 50)
                    sample_cases = mcs_summary_df.head(10)[['volatility_type', 'horizon', 'loss_metric', 'mcs_models', 'n_models_in_mcs']]
                    for _, row in sample_cases.iterrows():
                        print(f"{row['volatility_type']}, H={row['horizon']}, {row['loss_metric']}: {row['mcs_models']} ({row['n_models_in_mcs']} models)")
