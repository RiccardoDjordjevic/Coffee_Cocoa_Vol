...
def calculate_variable_importance(coefs_list, feature_names=None):
    ...
    Modified to handle both LASSO coefficient arrays and named Series
    """
    if not coefs_list:
        return None
    ...
    if isinstance(coefs_list[0], tuple):
        # LASSO format: coefs are tuples of (coef_array, intercept)
...
def rolling_window_forecast(data, features, target_var, horizon, n_folds=5, forecast_window=66, feature_set_name=None):
    """
    Implement rolling window forecasting with LASSO regression and proper window shifting as specified.
    ...
    if feature_set_name != 'pca':
        ...
        try:
            ...
            if perfect_collinear:
                ...
                # We'll keep these features as LASSO can handle multicollinearity
        except Exception as e:
    ...
    for fold in range(n_folds):
        ...
        try:
            ...
            lasso_cv = LassoCV(cv=5, random_state=random_seed, max_iter=10000, fit_intercept=True)
            lasso_cv.fit(X_train_scaled, y_train)
            
            # Get the optimal alpha
            optimal_alpha = lasso_cv.alpha_
            print(f"  Optimal alpha for fold {fold+1}: {optimal_alpha:.6f}")
            
            # Train LASSO with the optimal alpha
            model = Lasso(alpha=optimal_alpha, max_iter=10000, random_state=random_seed, fit_intercept=True)
            ...
            if 'model' in locals():
                print(f"  Model has intercept: {model.intercept_:.6f} and {sum(model.coef_ != 0)} non-zero coefficients")
...
if all_fold_losses:
    ...
    fold_losses_df.to_csv("lasso_clean_fold_losses_mcs.csv", index=False)
    print(f"\nFold losses for MCS testing saved to lasso_clean_fold_losses_mcs.csv")
    ...
    for horizon in horizons:
        ...
        if all_mses:
            ...
            print(f"  Best model for {target_var}, horizon {horizon}: "
                  f"LASSO_Clean with {best_feature_set} features, "
...
if mcs_detailed_results:
    ...
    mcs_df.to_csv("mcs_results_lasso_clean.csv", index=False)
    print(f"MCS results saved to mcs_results_lasso_clean.csv")
        ...
        if best_info['feature_set'] is not None:
            ...
            if coefs_data:
                ...
                print(f"\n  Variable Importance for {target_var}, horizon {horizon}, "
                      f"LASSO_Clean with {feature_set} features:")
...
best_models_df.to_csv("best_models_lasso_clean.csv", index=False)
        ...
        if importance is not None:
            ...
            for _, row in importance.iterrows():
                ...
                importance_results.append({
                    ...
                    'method': 'LASSO_Clean'  # Add the methodological approach
                })

# Create DataFrame of variable importance
if importance_results:
    importance_df = pd.DataFrame(importance_results)
    importance_df.to_csv("var_imp_lasso_clean.csv", index=False)  # Changed filename to LASSO_Clean
...
print("\nResults saved to:")
print("- best_models_lasso_clean.csv (best models summary)")
print("- var_imp_lasso_clean.csv (variable importance)")
print("- mcs_results_lasso_clean.csv (Model Confidence Set results)")
print("- lasso_clean_fold_losses_mcs.csv (fold-level losses for MCS testing)")
