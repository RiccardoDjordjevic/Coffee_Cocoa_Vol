...
def rolling_window_forecast(data, features, target_var, horizon, n_folds=5, forecast_window=66, feature_set_name=None):
    ...
    for fold in range(n_folds):
        ...
        try:
            ...
            print(f"  Optimal parameters for fold {fold+1}: alpha={optimal_alpha:.6f}, l1_ratio={optimal_l1_ratio:.6f}")
...
if all_fold_losses:
    ...
    fold_losses_df.to_csv("elasticnet_clean_fold_losses_mcs.csv", index=False)
    print(f"\nFold losses for MCS testing saved to elasticnet_clean_fold_losses_mcs.csv")
    ...
    for horizon in horizons:
        ...
        if all_mses:
            ...
            print(f"  Best model for {target_var}, horizon {horizon}: "
                  f"ElasticNet_Clean with {best_feature_set} features, "
...
if mcs_detailed_results:
    ...
    mcs_df.to_csv("mcs_results_en_clean.csv", index=False)
    print(f"MCS results saved to mcs_results_en_clean.csv")
        ...
        if best_info['feature_set'] is not None:
            ...
            if coefs_data:
                ...
                print(f"\n  Variable Importance for {target_var}, horizon {horizon}, "
                      f"ElasticNet_Clean with {feature_set} features:")
...
best_models_df.to_csv("best_models_en_clean.csv", index=False)  # Changed to en_clean
        ...
        if importance is not None:
            ...
            for _, row in importance.iterrows():
                ...
                importance_results.append({
                    ...
                    'method': 'ElasticNet_Clean'  # Add the methodological approach
                })

# Create DataFrame of variable importance
if importance_results:
    importance_df = pd.DataFrame(importance_results)
    importance_df.to_csv("var_imp_en_clean.csv", index=False)  # Changed to en_clean
...
results_df.to_csv("har_en_clean_all.csv", index=False)
print(f"Results saved to har_en_clean_all.csv")

print("\nResults saved to:")
print("- best_models_en_clean.csv (best models summary)")
print("- var_imp_en_clean.csv (variable importance)")
print("- mcs_results_en_clean.csv (Model Confidence Set results)")
print("- elasticnet_clean_fold_losses_mcs.csv (fold-level losses for MCS testing)")
print("- har_en_clean_all.csv (all model results)")
