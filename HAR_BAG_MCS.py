...
print("Loading data...")
try:
    data = pd.read_csv('data/clean_volatility.csv')
...
for feature_set_name in feature_sets:
    for target_var in feature_sets[feature_set_name]:
        ...
        if missing_features:
            print(f"Warning: The following features in {feature_set_name} for {target_var} are missing in the dataset: {missing_features}")
...
def calculate_variable_importance(estimators_list, feature_names=None):
    """
    Calculate variable importance from bagging ensemble by averaging the importance
    of each feature across all estimators and folds.
    
    For each fold, we average the coefficients across all bagging estimators,
    then normalize and average across folds.
    
    Modified to handle both tuple with feature_names and direct estimators list.
    """
    if not estimators_list:
        return None
    
    # Handle different formats of estimators lists
    if isinstance(estimators_list, tuple) and len(estimators_list) == 2:
        # PCA format: tuple of (estimators, feature_names)
        estimators, feature_names = estimators_list
    elif feature_names is None:
        print("Error: feature_names required for calculating variable importance")
        return None
    else:
    ...
    # Process each fold
    for fold_estimators in estimators:
        # Get average coefficients for all estimators in this fold
        fold_coefs = np.zeros(len(feature_names))
        count = 0
        
        for estimator in fold_estimators:
            if hasattr(estimator, 'coef_'):
                fold_coefs += np.abs(estimator.coef_)
                count += 1
        
        if count > 0:
            fold_coefs /= count  # Average across estimators
            
            # Normalize coefficients for this fold
            norm_factor = np.sum(fold_coefs)
            if norm_factor > 0:  # Avoid division by zero
                for i, var in enumerate(feature_names):
                    importance[var] += fold_coefs[i] / norm_factor
...
def rolling_window_forecast(data, features, target_var, horizon, n_folds=5, forecast_window=66, feature_set_name=None):
    """
    Implement rolling window forecasting with Bagging regression and proper window shifting as specified.
    ...
    if S <= 0:
        raise ValueError(f"Dataset too small for {n_folds} folds with {forecast_window} forecast window. Need more data.")
    ...
    if feature_set_name != 'pca':
        ...
        try:
            ...
            if perfect_collinear:
                ...
                # Bagging can handle multicollinearity
        except Exception as e:
    ...
    # Store model estimators for each fold
    all_estimators = []
    ...
    for fold in range(n_folds):
        ...
        try:
            ...
            # Use GridSearchCV to find optimal n_estimators for BaggingRegressor
            param_grid = {'n_estimators': [10, 20, 50, 100, 200]}
            base_model = LinearRegression()
            
            grid_search = GridSearchCV(
                BaggingRegressor(estimator=base_model, 
            ...
            print(f"  Optimal n_estimators for fold {fold+1}: {optimal_n_estimators}")
            
            # Train BaggingRegressor with the optimal parameters
            ...
            # Store model estimators for variable importance
            all_estimators.append(model.estimators_)
    ...
    # Return feature names along with estimators for PCA models
    if feature_set_name == 'pca' and pca_feature_names is not None:
        return mse, mae, qlike, (all_estimators, pca_feature_names), fold_losses
    else:
        return mse, mae, qlike, all_estimators, fold_losses
...
for target_var in ['parkinson_vol', 'garman_klass_vol']:
    ...
    for feature_set_name in feature_sets:
        ...
        for horizon in horizons:
            ...
            mse, mae, qlike, estimators_data, fold_losses = rolling_window_forecast(
            ...
            # Store estimators for variable importance calculation
            coefficients[target_var][feature_set_name][horizon] = estimators_data
...
if all_fold_losses:
    ...
    fold_losses_df.to_csv("bagging_clean_fold_losses_mcs.csv", index=False)
    print(f"\nFold losses for MCS testing saved to bagging_clean_fold_losses_mcs.csv")
    ...
    for horizon in horizons:
        ...
        if all_mses:
            ...
            print(f"  Best model for {target_var}, horizon {horizon}: "
                  f"Bagging_Clean with {best_feature_set} features, "
...
if mcs_detailed_results:
    ...
    mcs_df.to_csv("mcs_results_bag_clean.csv", index=False)
    print(f"MCS results saved to mcs_results_bag_clean.csv")
...
for target_var in ['parkinson_vol', 'garman_klass_vol']:
    for horizon in horizons:
        ...
        if best_info['feature_set'] is not None:
            ...
            # Get estimators for the best model
            estimators_data = coefficients[target_var][feature_set][horizon]
            
            if estimators_data:
                # Handle both PCA and regular feature formats
                if isinstance(estimators_data, tuple) and len(estimators_data) == 2:
                    # For PCA: unpack the tuple
                    estimators, feature_names = estimators_data
                else:
                ...
                importance = calculate_variable_importance(estimators, feature_names)
                var_importance_results[target_var][horizon] = importance
                
                print(f"\n  Variable Importance for {target_var}, horizon {horizon}, "
                      f"Bagging_Clean with {feature_set} features:")
...
best_models_df.to_csv("best_models_bag_clean.csv", index=False)  # Changed to bag_clean
        ...
        if importance is not None:
            ...
            for _, row in importance.iterrows():
                ...
                importance_results.append({
                    ...
                    'method': 'Bagging_Clean'  # Add the methodological approach
                })

# Create DataFrame of variable importance
if importance_results:
    importance_df = pd.DataFrame(importance_results)
    importance_df.to_csv("var_imp_bag_clean.csv", index=False)  # Changed to bag_clean
...
results_df.to_csv("har_bag_clean_all.csv", index=False)
print(f"Results saved to har_bag_clean_all.csv")

print("\nResults saved to:")
print("- best_models_bag_clean.csv (best models summary)")
print("- var_imp_bag_clean.csv (variable importance)")
print("- mcs_results_bag_clean.csv (Model Confidence Set results)")
print("- bagging_clean_fold_losses_mcs.csv (fold-level losses for MCS testing)")
print("- har_bag_clean_all.csv (all model results)")
