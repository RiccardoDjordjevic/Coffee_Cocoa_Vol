...
if all_fold_losses:
    ...
    fold_losses_df.to_csv("bagging_fold_losses_mcs.csv", index=False)
    print(f"\nFold losses for MCS testing saved to bagging_fold_losses_mcs.csv")
    ...
    for horizon in horizons:
        ...
        if all_mses:
            ...
            print(f"  Best model for {target_var}, horizon {horizon}: "
                  f"Bagging with {best_feature_set} features, "
...
if mcs_detailed_results:
    ...
    mcs_df.to_csv("mcs_results_bag.csv", index=False)
    print(f"MCS results saved to mcs_results_bag.csv")
        ...
        if best_info['feature_set'] is not None:
            ...
            if estimators_data:
                ...
                print(f"\n  Variable Importance for {target_var}, horizon {horizon}, "
                      f"Bagging with {feature_set} features:")
...
best_models_df.to_csv("best_models_bag.csv", index=False)  # Changed to bag
        ...
        if importance is not None:
            ...
            for _, row in importance.iterrows():
                ...
                importance_results.append({
                    ...
                    'method': 'Bagging'  # Add the methodological approach
                })

# Create DataFrame of variable importance
if importance_results:
    importance_df = pd.DataFrame(importance_results)
    importance_df.to_csv("var_imp_bag.csv", index=False)  # Changed to bag
...
results_df.to_csv("har_bag_all.csv", index=False)
print(f"Results saved to har_bag_all.csv")

print("\nResults saved to:")
print("- best_models_bag.csv (best models summary)")
print("- var_imp_bag.csv (variable importance)")
print("- mcs_results_bag.csv (Model Confidence Set results)")
print("- bagging_fold_losses_mcs.csv (fold-level losses for MCS testing)")
print("- har_bag_all.csv (all model results)")
