...
# Create lagged features for parkinson_vol with diagnostics
...
if all_fold_losses:
    ...
    fold_losses_df.to_csv("lasso_fold_losses_mcs.csv", index=False)
    print(f"\nFold losses for MCS testing saved to lasso_fold_losses_mcs.csv")
    ...
    for horizon in horizons:
        ...
        if all_mses:
            ...
            print(f"  Best model for {target_var}, horizon {horizon}: "
                  f"LASSO with {best_feature_set} features, "
...
if mcs_detailed_results:
    ...
    mcs_df.to_csv("mcs_results_lasso.csv", index=False)
    print(f"MCS results saved to mcs_results_lasso.csv")
        ...
        if best_info['feature_set'] is not None:
            ...
            if coefs_data:
                ...
                print(f"\n  Variable Importance for {target_var}, horizon {horizon}, "
                      f"LASSO with {feature_set} features:")
...
best_models_df.to_csv("best_models_lasso.csv", index=False)
        ...
        if importance is not None:
            ...
            for _, row in importance.iterrows():
                ...
                importance_results.append({
                    ...
                    'method': 'LASSO'  # Add the methodological approach
                })

# Create DataFrame of variable importance
if importance_results:
    importance_df = pd.DataFrame(importance_results)
    importance_df.to_csv("var_imp_lasso.csv", index=False)  # Changed filename to LASSO
...
print("\nResults saved to:")
print("- best_models_lasso.csv (best models summary)")
print("- var_imp_lasso.csv (variable importance)")
print("- mcs_results_lasso.csv (Model Confidence Set results)")
print("- lasso_fold_losses_mcs.csv (fold-level losses for MCS testing)")
